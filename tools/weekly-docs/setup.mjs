...
if (driveUrl) {
  ...
  writeText(linkFile, [
    "아래 링크를 아이패드로 보내세요(카카오톡 '나와의 채팅', 메모 등).",
    "",
    "· 홈 화면에 추가한 '레슨 로그' 앱으로 기록하는 경우:",
    "  링크를 길게 눌러 복사 → 앱 [설정] → 구글 드라이브 자동 백업 → '웹 앱 URL' 칸에 붙여넣기 → [저장하고 지금 백업]",
    "  (사파리와 홈 화면 앱은 기록이 따로 저장되므로, 링크를 사파리에서 열면 앱에는 연결되지 않습니다)",
    "· 사파리에서 기록하는 경우: 링크를 사파리에서 열면 자동으로 연결됩니다.",
...
[3] 아이패드 앱 연결
  '아이패드 연동 링크.txt' 안의 링크를 카카오톡 '나와의 채팅'이나 메모로 아이패드에 보내세요.
  - 홈 화면 앱으로 기록한다면: 링크를 복사해 앱 [설정] → '웹 앱 URL' 칸에 붙여넣고 [저장하고 지금 백업]
    (사파리와 홈 화면 앱은 기록이 따로 저장됩니다)
  - 사파리에서 기록한다면: 링크를 사파리에서 열면 자동으로 연결됩니다.
